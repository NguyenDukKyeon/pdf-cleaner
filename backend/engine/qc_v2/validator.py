...
def validate_output(input_pdf: str | Path, output_pdf: str | Path, report, *, max_outside_change_ratio: float = 0.08, pixel_change_threshold: int = 24, dpi: int = 96) -> QCReport:
    input_pdf = Path(input_pdf); output_pdf = Path(output_pdf)
    reasons: list[str] = []; failed_pages: list[int] = []
    metadata = dict(getattr(report, "metadata", {}) or {})
    raw_regions = metadata.get("watermark_regions") or []
    regions: list[tuple[float, float, float, float]] = []
    for item in raw_regions:
        try:
            if isinstance(item, dict): item = (item["x0"], item["y0"], item["x1"], item["y1"])
            regions.append(tuple(float(v) for v in item))
        except Exception: continue
    try:
        src = fitz.open(str(input_pdf)); out = fitz.open(str(output_pdf))
    ...
    try:
        ...
        for index in range(src.page_count):
            ...
            if abs(a.width-b.width) > 0.5 or abs(a.height-b.height) > 0.5:
                geometry_ok=False; failed_pages.append(index)
        if not geometry_ok:
            reasons.append("page geometry changed")
            return QCReport(False, True, False, 1.0, tuple(failed_pages), tuple(reasons), 1.0)
        outside_changed=outside_total=changed_total=pixel_total=0
        for index in range(src.page_count):
            a=_render_gray(src[index],dpi); b=_render_gray(out[index],dpi)
            ...
            allowed=_allowed_mask(h,w,regions); outside=~allowed
            changed_total += int(changed.sum()); pixel_total += int(changed.size)
            outside_changed += int((changed & outside).sum()); outside_total += int(outside.sum())
            ratio=float((changed & outside).sum())/max(1,int(outside.sum()))
            if ratio > max_outside_change_ratio: failed_pages.append(index)
        outside_ratio=outside_changed/max(1,outside_total); changed_ratio=changed_total/max(1,pixel_total)
        if outside_ratio > max_outside_change_ratio:
            reasons.append(f"outside-mask visual change {outside_ratio:.4f} exceeds {max_outside_change_ratio:.4f}")
        return QCReport(page_count_ok and geometry_ok and outside_ratio <= max_outside_change_ratio, page_count_ok, geometry_ok, float(outside_ratio), tuple(sorted(set(failed_pages))), tuple(reasons), float(changed_ratio))
    finally:
        src.close(); out.close()
