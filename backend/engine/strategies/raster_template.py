...
class RasterTemplateStrategy:
    def execute(self, input_pdf: Path, output_pdf: Path, plan: ProcessingPlan, *, log: Callable[[str], None] | None = None, progress: Callable[[int, int, str], None] | None = None, should_cancel: Callable[[], bool] | None = None) -> StrategyResult:
        ...
        try:
            _, samples = load_native_samples(doc, max_samples=8)
            if len(samples) < 2:
            ...
            processed_xrefs = set(); total = doc.page_count
            for index in range(total):
                ...
                if native.xref in processed_xrefs:
                    if progress: progress(index + 1, total, "native image reused")
                    continue
                ...
                if model.mask.shape != rgb.shape[:2]:
                    import cv2
                    mask = cv2.resize(model.mask.astype(np.uint8), (rgb.shape[1], rgb.shape[0]), interpolation=cv2.INTER_NEAREST) > 0
                    template = cv2.resize(model.template_gray.astype(np.float32), (rgb.shape[1], rgb.shape[0]), interpolation=cv2.INTER_LINEAR)
                    active_model = RasterWatermarkModel(mask, template, model.confidence, model.sample_count, model.marker_id)
                cleaned, pixels = repair_with_model(rgb, active_model)
                changed_pixels += pixels
                if pixels:
                    bio = BytesIO(); Image.fromarray(cleaned).save(bio, format="PNG", optimize=False)
                    doc[index].replace_image(native.xref, stream=bio.getvalue())
                    changed_pages += 1
                processed_xrefs.add(native.xref)
                if progress: progress(index + 1, total, "applying document watermark model")
            output_pdf = Path(output_pdf); output_pdf.parent.mkdir(parents=True, exist_ok=True)
            doc.save(str(output_pdf), garbage=4, deflate=True)
            return StrategyResult(removed_items=changed_pixels, changed_pages=changed_pages, rasterized_pages=0, saved_to=str(output_pdf), native_image_pages=native_pages, ocr_calls=0)
        finally:
            doc.close()
