...
def test_tailieuonthi_marker_uses_signature_specific_tdm_guided_cleanup(tmp_path, monkeypatch):
    ...
    def fake_guided(input_pdf, output_pdf, **kwargs):
        ...
        return TdmGuidedResult(
            ...
            footer_residual_score=0.018,
            footer_protected_change_ratio=0.0008,
            page_footer_residual_scores=(0.012, 0.018),
        )
    ...
    result = RasterTemplateStrategy().execute(src, out, plan, footer_cleanup="deep")

    assert len(calls) == 1
    assert calls[0][2].get("footer_cleanup") == "deep"
    ...
    assert result.metadata['footer_cleanup_level'] == "deep"
    assert result.metadata['footer_residual_score'] == 0.018
    assert result.metadata['footer_protected_change_ratio'] == 0.0008
    assert result.metadata['page_footer_residual_scores'] == [0.012, 0.018]


def test_raster_template_strategy_defaults_footer_cleanup_to_auto(tmp_path, monkeypatch):
    import shutil
    import backend.engine.strategies.raster_template as raster_module
    from backend.engine.raster.tdm_guided import TdmGuidedResult
    ...
    assert len(calls) == 1
    assert calls[0][2].get("footer_cleanup") == "auto"
    assert result.metadata['footer_cleanup_level'] == "standard"
